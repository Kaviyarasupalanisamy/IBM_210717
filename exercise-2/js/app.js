...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    if (typeof(Storage) !== "undefined" && localStorage.getItem("tasks") != null) {
        ...
        if (todo) {
            todoFiltered = todo.filter((val => val.completed == false && val.deleted == false))
            ...
            completed = todo.filter((val => val.completed == true && val.deleted == false))
            completed.map((val) => {
    ...
    if ( getLocalstorage() == null) {
        ...
        for (var i = 0; i < incompleteTasksHolder.children.length; i++) {
            ...
            if (tasks.findIndex(a => a.name == taskText && a.completed == true) != -1) {
        ...
        for (var i = 0; i < completedTasksHolder.children.length; i++) {
            ...
            var taskText = listItem.querySelector("label").innerText;
            if (tasks.findIndex(a => a.name == taskText && a.completed == false) != -1) {
